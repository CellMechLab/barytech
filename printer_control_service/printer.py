...
class Printer:
    ...
    def get_status_combined(self) -> tuple:
        """
        Query position (M114) and temperature (M105) in a single serial
        session, reusing one lock acquisition instead of two.

        Returns:
            (Position, dict)  — same types as get_position() and get_temperature()

        This halves the worst-case serial round-trip time compared to calling
        both methods separately (one lock + two commands instead of two locks).
        """
        with self._lock:
            self._require_connected()
            self._ser.reset_input_buffer()
            ...
            while time.time() < deadline:
                ...
                for axis in ("X", "Y", "Z", "E"):
                    ...
                    if m:
                        parsed[axis] = float(m.group(1))
                if parsed:
                    pos = Position(**{k: parsed.get(k, 0.0) for k in ("X", "Y", "Z", "E")})
                    break
            if pos is None:
                raise PrinterTimeoutError("M114 did not return position data in time")

            # Drain remaining lines until 'ok' so the buffer is clean for M105
            while time.time() < deadline:
                line = self._ser.readline().decode(errors="ignore").strip()
                if line:
                    logger.debug("RX  %s", line)
