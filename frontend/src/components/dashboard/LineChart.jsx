...
const LineChart = forwardRef(({ dataset = "force" }, ref) => {
  ...
  // Transform data buffer to series format, plotting all points without averaging
  const transformData = (dataBuffer) => {
    if (!Array.isArray(dataBuffer)) return { series1: [], series2: [] };
  ...
  const downloadDataBufferAsCSV = () => {
    ...
    const csvContent = [["Timestamp", "Displacement", "Force"]];
  ...
  useEffect(() => {
    ...
    const drawChart = () => {
      ...
      // Updated Y-axis ranges to match actual sensor data values
      // Force: 0-300 N (covers full range ~25-275 N)
      // Displacement: 0-120 m (covers full range ~10-110 m)
      const defaultYDomain = dataset === "force" ? [0, 300] : [0, 120];
        ...
        tooltip
          .html(
            `${label}<br>Date: ${d3.timeFormat("%Y-%m-%d %H:%M:%S")(
              d.date
            )}<br>Value: ${d.value.toFixed(3)} ${label === "Force" ? "N" : "m"}`
          )
          .style("display", "block");
      }
