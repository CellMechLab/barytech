#!/usr/bin/env python3
"""
Frontend-2 Publisher
A simple MQTT publisher that sends data specifically for testing frontend-2 with device-specific topic optimization
"""
...
class Frontend2Publisher:
    def __init__(self):
        self.client = mqtt.Client(client_id="frontend2_publisher", protocol=mqtt.MQTTv311, clean_session=False)
        self.client.on_connect = self.on_connect
        self.client.on_publish = self.on_publish
        self.client.on_disconnect = self.on_disconnect
        
        # Publisher settings
        self.device_id = "frontend2_device"
    ...
    def on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            print(f"✅ Frontend-2 Publisher connected to MQTT broker")
    ...
    def on_disconnect(self, client, userdata, rc):
        print(f"🔌 Frontend-2 Publisher disconnected")
        
    def generate_data_point(self):
        """Generate a realistic data point for frontend-2"""
        timestamp = datetime.utcnow().isoformat()
        
        # Generate realistic displacement and force values
        # Simulate a sensor reading with some noise
        base_displacement = 15.0 + (self.message_count * 0.15)  # Gradually increasing, different from frontend-1
        noise_displacement = random.uniform(-0.8, 0.8)
        displacement = base_displacement + noise_displacement
        
        # Force correlates with displacement but with some variation
        base_force = displacement * 3.0 + random.uniform(-1.5, 1.5)  # Different scaling from frontend-1
    ...
    def publish_data(self):
        ...
        print(f"\n📊 Frontend-2 Publisher Statistics:")
    ...
    def start(self):
        ...
        try:
            ...
            print(f"\n⏹️  Stopping Frontend-2 Publisher...")
            self.is_running = False
            self.client.disconnect()
...
def main():
    """Main function to run the Frontend-2 Publisher"""
    print("=" * 60)
    print("🚀 Frontend-2 Publisher")
    print("=" * 60)
    print("This publisher sends data specifically for testing frontend-2 with device-specific topic optimization")
    print("Make sure:")
